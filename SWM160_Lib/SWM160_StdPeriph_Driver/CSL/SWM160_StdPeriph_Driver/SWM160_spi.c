...
void SPI_Init(SPI_TypeDef * SPIx, SPI_InitStructure * initStruct)
{
	...
	SPIx->CTRL &= ~(SPI_CTRL_FFS_Msk | SPI_CTRL_CPHA_Msk | SPI_CTRL_CPOL_Msk | SPI_CTRL_DMAEN_Msk |
					SPI_CTRL_DSS_Msk | SPI_CTRL_MSTR_Msk | SPI_CTRL_FAST_Msk | SPI_CTRL_CLKDIV_Msk | SPI_CTRL_SSN_H_Msk);
	...
	if(initStruct->clkDiv < 8)
	{
		SPIx->CTRL |= (initStruct->clkDiv << SPI_CTRL_CLKDIV_Pos);
	}
	else
	{
		SPIx->CTRL |= (1 << SPI_CTRL_FAST_Pos);
	}
	...
	{
	case ((uint32_t)SPI0):	
		if(initStruct->RXHFullIEn | initStruct->RXFullIEn | initStruct->TXHFullIEn | initStruct->TXEmptyIEn)
		{
