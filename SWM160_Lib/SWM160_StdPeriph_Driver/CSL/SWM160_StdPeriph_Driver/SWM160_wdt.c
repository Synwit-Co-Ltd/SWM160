...
*			uint32_t mode			WDT_MODE_RESET   		   计数器减小到0时产生复位
*									WDT_MODE_INTERRUPT		   计数器减小到LOAD/4时产生中断
*									WDT_MODE_INTERRUPT_RESET   计数器减小到LOAD/4时产生中断，减小到0时产生复位
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
void WDT_Init(WDT_TypeDef * WDTx, uint32_t peroid, uint32_t mode)
{
	...
	else if(mode == WDT_MODE_INTERRUPT)
	{
		WDT_INTClr(WDTx);
