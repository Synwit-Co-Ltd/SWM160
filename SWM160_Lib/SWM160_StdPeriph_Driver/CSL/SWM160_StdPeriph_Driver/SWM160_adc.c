...
void ADC_Init(ADC_TypeDef * ADCx, ADC_InitStructure * initStruct)
{
	...
	*((volatile uint32_t *)0x40000884) &= ~3;
	*((volatile uint32_t *)0x40000884) |=  2;	// 10位
	
	ADC->CTRL1 &= ~(0xFF <<  8);
	ADC->CTRL1 |= (((SYS->CHIP_ID[0] >>  9) & 0x7F) <<  8);
	ADC->CTRL1 &= ~(0x0F << 16);
	ADC->CTRL1 |= (((SYS->CHIP_ID[0] >> 28) & 0x0F) << 16);
	
	ADCx->CALIBSET = SYS->CHIP_ID[2];
	ADCx->CALIBEN = (1 << ADC_CALIBEN_OFFSET_Pos) | (1 << ADC_CALIBEN_K_Pos);
	
	ADCx->CTRL1 &= ~ADC_CTRL1_CLKSRC_Msk;
	ADCx->CTRL1 |= (initStruct->clk_src << ADC_CTRL1_CLKSRC_Pos);
	...
	ADCx->CTRL2 &= ~(ADC_CTRL2_ADCEVCM_Msk | ADC_CTRL2_PGAEVCM_Msk | ADC_CTRL2_PGAGAIN_Msk | ADC_CTRL2_VCMSEL_Msk | ADC_CTRL2_CLKDIV1_Msk | ADC_CTRL2_CLKDIV2_Msk);
 	ADCx->CTRL2 |=  (1                    << ADC_CTRL2_ADCEVCM_Pos) |
					(PGA_VCM_EXTERNAL     << ADC_CTRL2_PGAEVCM_Pos) |
					(7                    << ADC_CTRL2_PGAGAIN_Pos) |
					(7                    << ADC_CTRL2_VCMSEL_Pos)  |
					(0                    << ADC_CTRL2_CLKDIV1_Pos) |
					(initStruct->clk_div  << ADC_CTRL2_CLKDIV2_Pos);
	
	ADCx->CTRL &= ~(((uint32_t)3 << 30) | (3 << 28));		//一次有效采样期间，实际采样2次；第2个有效latch_clk对应的数据
	ADCx->CTRL |=  (((uint32_t)1 << 30) | (1 << 28));
